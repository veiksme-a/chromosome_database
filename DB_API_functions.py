...
information, hence the functions are used tohether. The decision to make two
...
def gen_info(middle_db_input):
    ...
    query_t0 = "from chr_db.locus l, chr_db.cds c, chr_db.accession a where a.locus_id=l.id and c.locus_id=l.id and "
    ...
    cyt_loc_cds = "c.gene_name, c.product_name"
    ...
    if middle_db_input["type"]==0 or middle_db_input["type"]==1:
        ...
        db_middle_output =db_middle_output + unit1 + unit2
...
def chr_codon_usage():
    ...
    query = "select * from chr_db.codon_usage"
...
def restriction_sites(middle_db_input):
    ...
    """Obtain the information on teh restriction site location in a particular
    ...
    query1 = "select s.start_position, s.end_position, e.recogn_seq from chr_db.locus l, chr_db.restriction_enzyme e, chr_db.restriction_sites s, chr_db.cds c where c.locus_id=l.id and s.locus_id=l.id and s.re_id=e.id and "
    query2 = "select c.gene_name, c.product_name, c.seq_location, l.whole_seq from chr_db.locus l, chr_db.cds c where c.locus_id=l.id and "
    
    # Queries if the user is searching using the locus accession number.
    query3 = "select s.start_position, s.end_position, e.recogn_seq from chr_db.locus l, chr_db.restriction_enzyme e, chr_db.restriction_sites s, chr_db.accession a where a.locus_id=l.id and s.locus_id=l.id and s.re_id=e.id and "
    query4 = "select l.whole_seq from chr_db.locus l, chr_db.accession a where a.locus_id=l.id and "
    query5 = "select c.gene_name, c.product_name, c.seq_location from chr_db.locus l, chr_db.cds c, chr_db.accession a where a.locus_id=l.id and c.locus_id=l.id and "
    ...
    if middle_db_input["enzyme"] in preset:
        ...
        if middle_db_input["type"]==0 or middle_db_input["type"]==1:
            db_middle_output = [middle_db_input] + r_sites + sequence
        if middle_db_input["type"]==2:
            cursor3 = db.cursor()
            q_z = cursor3.execute(query_z)
            cds = cursor3.fetchall()
            db_middle_output = [middle_db_input] + r_sites + sequence + cds
